...
app.put('/job-postings/:id', (req, res) => {
    const jobId = req.params.id;
    const { job_title, expiry_date,required_skills,job_description, experience, salaryRange, jobType, location} = req.body;
    const sql = 'UPDATE job_postings SET job_title = ?,job_description=? ,required_skills =? ,experience = ?,salary_range = ?, expiry_date = ?, job_type = ?, location = ? WHERE id = ?';

    db.query(sql, [job_title,job_description,required_skills,experience, salaryRange, expiry_date, jobType, location, jobId], (err, results) => {
        if (err) {
...
app.post('/job-postings', uploaddd.single('file'), (req, res) => {
    const { jobTitle, jobDescription, requiredSkills, experience, salaryRange, expiryDate, jobType, postedDate,recruiterId, jobLocation,company } = req.body;
    ...
    const sql = `INSERT INTO job_postings (job_title, job_description, required_skills, experience, salary_range, expiry_date, job_type, posted_date, recruiter_id, location, image_url,company)
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?)`;

    db.query(sql, [jobTitle, jobDescription, requiredSkills, experience, salaryRange, expiryDate, jobType, postedDate,recruiterId, jobLocation, imageUrl,company], (err, result) => {
        if (err) {
